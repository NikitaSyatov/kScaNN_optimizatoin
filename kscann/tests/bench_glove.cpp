...
// .fvecs loader (returns flat vector)
...
// Compute exact top‑k nearest neighbours using brute force L2 distance
...
// Compute recall@k: |intersection(approx, exact)| / k
...
int main(int argc, char** argv) {
    if (argc < 2) {
        std::cerr << "Usage: " << argv[0] << " <path_to_glove100_base.fvecs> [rairs] [num_queries]\n";
        return 1;
    }
    std::string base_file = argv[1];
    bool use_rairs = (argc >= 3 && std::string(argv[2]) == "rairs");
    int num_queries = (argc >= 4) ? std::stoi(argv[3]) : 100;
    ...
    std::cout << "Loaded " << num_base << " points of dimension " << dim << "\n";
    num_queries = std::min(num_queries, (int)num_base);

    kscann::IndexConfig cfg;
    cfg.num_clusters = 200;          // smaller clusters → better recall
    cfg.num_pq_subspaces = 10;       // keep as is
    cfg.num_pq_centroids = 256;
    cfg.nprob = 100;
    cfg.reorder = 1000;               // re‑rank many candidates
    cfg.use_rairs = use_rairs;
    if (use_rairs) {
        cfg.k_assign = 2;        // good trade‑off between recall and speed
    ...
    // Prepare queries (first num_queries points)
    ...
    // Pre‑compute exact top‑k for all queries (k = 10)
    int k = 10;
    ...
    // Search and measure recall + time
    ...
    for (int i = 0; i < num_queries; ++i) {
        ...
        for (auto id : approx) if (id == i) found_self = true;
        if (!found_self) std::cout << "Query " << i << " did not retrieve itself\n";
    }
